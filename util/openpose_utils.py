...
def draw_pose_from_kpt_smpl(B_coor, resize_param=None, org_size=None, radius=2, affine=None, LIMB_SEQ=None):
    pose_joints = obtain_2d_coords_smpl(B_coor, resize_param, org_size, affine)
    pose_body = pose_joints['body']
    pose_body = pose_body.astype(np.int)
    if resize_param is not None:
        im_size = resize_param
...
def obtain_2d_coords_smpl(B_coor, resize_param=None, org_size=None, affine=None):
    pose_joints=dict()
    pose = B_coor["pose_keypoints_2d"]
    coor_x, coor_y, score = [], [], []
    for kpt in pose:
