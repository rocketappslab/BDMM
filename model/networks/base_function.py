...
class _SPADE(nn.Module):
    ...
    def forward(self, x, segfeature):
        ...
        segfeature = F.interpolate(segfeature, size=x.size()[2:], mode='bilinear', align_corners=True)
...
class EncoderBlockSingleConv(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.LeakyReLU(),
                 use_spect=False, use_coord=False):
        super(EncoderBlockSingleConv, self).__init__()
        ...
        if type(norm_layer) == type(None):
            self.model = nn.Sequential(nonlinearity, conv1)
        else:
...
class EncoderBlockSingleConvResidualBlock(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.LeakyReLU(),
                 use_spect=False, use_coord=False):
        super(EncoderBlockSingleConvResidualBlock, self).__init__()
...
class ResBlock(nn.Module):
    ...
    """
    def __init__(self, input_nc, output_nc=None, hidden_nc=None, norm_layer='batch', nonlinearity='ReLU',
                learnable_shortcut=False, use_spect=False, use_coord=False):
        super(ResBlock, self).__init__()
        if isinstance(norm_layer, str):
            norm_layer = get_norm_layer(norm_type=norm_layer)
            nonlinearity = get_nonlinearity_layer(activation_type=nonlinearity)
...
class SPADE(nn.Module):
    def __init__(self, norm_nc, label_nc, norm_type=None):
        super().__init__()
    ...
    def forward(self, normalized, segmap):

        # Part 1. generate parameter-free normalized activations
        # normalized = self.param_free_norm(x)

        # Part 2. produce scaling and bias conditioned on semantic map
        segmap = F.interpolate(segmap, size=normalized.size()[2:], mode='bilinear', align_corners=False)
