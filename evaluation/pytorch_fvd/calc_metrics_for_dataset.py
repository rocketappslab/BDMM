...
from evaluation.pytorch_fvd import metric_main
from evaluation.pytorch_fvd import metric_utils
from evaluation.pytorch_fvd.torch_utils import training_stats
from evaluation.pytorch_fvd.torch_utils import custom_ops
...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        if rank == 0:
            report = metric_main.report_metric(result_dict, run_dir=args.run_dir)
...
def calc_metrics_for_dataset(ctx, metrics, real_data_path, fake_data_path, mirror, resolution, gpus, verbose, use_cache: bool, num_runs: int):
    ...
    args.dataset_kwargs = dnnlib.EasyDict(
        class_name='evaluation.pytorch_fvd.training.dataset.VideoFramesFolderDataset',
        path=real_data_path,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if args.num_gpus == 1:
            report = subprocess_fn(rank=0, args=args, temp_dir=temp_dir)
        else:
